...
// definition of the properties 
  ...
  { global_variable * _CL_obj = (Reader.arrow = (global_variable *) Core._global_variable->instantiate("arrow",claire.it));
    ...
    { global_variable * g0141 = _CL_obj; 
      ...
      { keyword * _CL_obj = ((keyword *) new_object_class(Kernel._keyword));
        (_CL_obj->name = symbol_I_string2(CSTRING("->")));
  ...
  { (Reader._meta_reader = ClaireClass::make("meta_reader",Kernel._thing,claire.it));
    ...
    CL_ADD_SLOT(Reader._meta_reader,meta_reader,Kernel.external,external,Kernel._string,_string_(CSTRING("toplevel")));
  ...
  { global_variable * _CL_obj = (Reader.OR = (global_variable *) Core._global_variable->instantiate("OR",claire.it));
    (_CL_obj->range = Kernel._any);
    (_CL_obj->value = _oid_(new_thing_class(Reader._delimiter,symbol_I_string(CSTRING("|"),claire.it))));
    close_global_variable(_CL_obj);
    } 
  ...
  { (Reader.reader = (meta_reader *) Reader._meta_reader->instantiate("reader",claire.it));
    ...
    (Reader.reader->bracket = _oid_(new_thing_class(Reader._delimiter,symbol_I_string2(CSTRING("]")))));
    (Reader.reader->paren = _oid_(new_thing_class(Reader._delimiter,symbol_I_string2(CSTRING(")")))));
    (Reader.reader->comma = _oid_(new_thing_class(Reader._delimiter,symbol_I_string2(CSTRING(",")))));
    (Reader.reader->curly = _oid_(new_thing_class(Reader._delimiter,symbol_I_string2(CSTRING("}")))));
    ;} 
  ...
  { global_variable * _CL_obj = (Reader.STDOUT = (global_variable *) Core._global_variable->instantiate("stdout",claire.it));
    ...
    (_CL_obj->value = get_symbol(symbol_I_string(CSTRING("STDOUT"),claire.it)));
    close_global_variable(_CL_obj);
    } 
  ...
  { global_variable * _CL_obj = (Reader.STDIN = (global_variable *) Core._global_variable->instantiate("stdin",claire.it));
    (_CL_obj->range = Kernel._port);
    (_CL_obj->value = get_symbol(symbol_I_string(CSTRING("STDIN"),claire.it)));
    close_global_variable(_CL_obj);
    } 
