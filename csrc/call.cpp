/***** CLAIRE Compilation of file /Users/ycaseau/claire/v3.5/src/meta/call.cl 
...
{ GC_BIND;
  { OID Result = 0;
    ...
    else { OID  V_CL0023;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[101] ~S is not a variable")),
...
{ GC_BIND;
  princ_string(CSTRING("("));
  printbox_bag3(GC_OBJECT(list,self->args),CSTRING(" & "));
  princ_string(CSTRING(")"));
  GC_UNBIND;} 
...
{ { OID Result = 0;
    if (INHERIT(OWNER(self),Language._Variable))
     ...
     { { Cint  i = 1;
        Cint  g0033 = OBJECT(bag,self)->length;
        { while ((i <= g0033))
...
Cint  occurrence_any(OID self,Variable *x)
{ { Cint Result = 0;
    if (INHERIT(OWNER(self),Language._Variable))
     ...
     { Cint  n = 0;
      { Cint  i = 1;
        Cint  g0034 = OBJECT(bag,self)->length;
        { while ((i <= g0034))
  ...
  { OID Result = 0;
    if (INHERIT(OWNER(self),Kernel._bag))
     { bag * l = GC_OBJECT(bag,copy_bag(OBJECT(bag,self)));
      { Cint  i = 1;
        Cint  g0035 = OBJECT(bag,self)->length;
        { OID gc_local;
          while ((i <= g0035))
