  ...
  { OID Result = 0;
    { OID  val = GC_OID(eval_any(GC_OID(self->value)));
      ...
      if ((n > 0) && 
          (n == ClEnv->step_I))
       { (ClEnv->trace_I = 0);
        princ_string(CSTRING("start: let "));
...
{ GC_BIND;
  { Cint  _Zl = Core.pretty->index;
    set_level_integer(1);
    princ_string(CSTRING("when "));
    ppvariable_Variable(GC_OBJECT(Variable,self->var));
    princ_string(CSTRING(" := "));
    printexp_any(GC_OID(self->value),CFALSE);
    princ_string(CSTRING(" in "));
    lbreak_integer(2);
  ...
  { OID Result = 0;
    { OID  val = GC_OID(eval_any(GC_OID(self->value)));
      ...
      if ((n > 0) && 
          (n == ClEnv->step_I))
       { (ClEnv->trace_I = 0);
        princ_string(CSTRING("start: when "));
        print_any(GC_OID(_oid_(self->var)));
        princ_string(CSTRING(" := "));
        print_any(val);
        princ_string(CSTRING(" ...\n"));
  ...
  { OID Result = 0;
    { OID  x = GC_OID(eval_any(GC_OID(self->set_arg)));
      ...
      if ((n > 0) && 
          (n == ClEnv->step_I))
       { (ClEnv->trace_I = 0);
        princ_string(CSTRING("start: for "));
        print_any(GC_OID(_oid_(self->var)));
        princ_string(CSTRING(" in "));
        print_any(x);
        princ_string(CSTRING(" ...\n"));
      ...
      { ClaireHandler c_handle = ClaireHandler();
        ...
        { if (INHERIT(OWNER(x),Kernel._class))
           ...
           { Cint  y = OBJECT(Interval,x)->arg1;
            Cint  g0046 = OBJECT(Interval,x)->arg2;
            { OID gc_local;
              Result= _oid_(CFALSE);
              while ((y <= g0046))
              { GC_LOOP;
          ...
          else { OID  V_CL0047;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[136] ~S is not a collection !")),
...
{ GC_BIND;
  if (self->other == Kernel.ctrue)
   princ_string(CSTRING("forall"));
  else if (self->other == Kernel.cfalse)
   princ_string(CSTRING("exists"));
  else princ_string(CSTRING("some"));
...
{ GC_BIND;
  princ_string(CSTRING("case "));
  print_any(GC_OID(self->var));
  princ_string(CSTRING(" "));
  lbreak_integer(1);
  princ_string(CSTRING("("));
  { Cint  n = 1;
    Cint  m = self->args->length;
    (Core.pretty->index = (Core.pretty->index+1));
    { OID gc_local;
      while ((n <= m))
      { GC_LOOP;
...
{ GC_BIND;
  princ_string(CSTRING("try "));
  print_any(GC_OID(self->arg));
  princ_string(CSTRING(" "));
  lbreak_integer(0);
  princ_string(CSTRING("catch "));
...
{ GC_BIND;
  if ((boolean_I_any(_oid_(self->args)) != CTRUE) || 
   ...
   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("Syntax error: ~S")),
  ...
  { OID Result = 0;
    { list * l = GC_OBJECT(list,self->args);
      ...
      if (equal(_oid_(Kernel._string),_oid_(OWNER(s))) != CTRUE)
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[102] the first argument in ~S must be a string")),
        _oid_(list::alloc(1,_oid_(self))))));
      else { Cint  i = 2;
            ...
            { GC_LOOP;
              { OID  m = GC_OID((*Kernel.nth)(s,
                  GC_OID((*Core._plus)(n,
                    1))));
                if (i > l->length)
                 close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[103] not enough arguments in ~S")),
                  _oid_(list::alloc(1,_oid_(self))))));
                ...
                if (_oid_(_char_('A')) == m)
                 ...
                 { OID  fv = GC_OID(OPT_EVAL((*(l))[i]));
                  ...
                  Cint  j = (integer_I_char(_char_(string_v(s)[(*Core._plus)(n,
                    2) - 1]))-48);
    ...
    { list * a = GC_OBJECT(list,self->args);
      if ((a->length > 0) && 
       ...
       { ClairePort * p = (use_as_output_port(ClEnv->ctrace));
        ...
        if (0 <= ClEnv->debug_I)
         close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("Assertion Violation")),
          _oid_(Kernel.nil))));
        Result = _oid_(Kernel.emptySet);
        } 
      else Result = Kernel.cfalse;
...
{ GC_BIND;
  if (self->args->length != 1)
   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[104] Syntax error with ~S (one arg. expected)")),
    _oid_(list::alloc(1,_oid_(self))))));
  { OID Result = 0;
    { ClaireHandler c_handle = ClaireHandler();
      if ERROR_IN 
