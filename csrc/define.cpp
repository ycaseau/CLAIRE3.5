/***** CLAIRE Compilation of file /Users/ycaseau/claire/v3.5/src/meta/define.cl 
  ...
  { OID gc_local;
    ...
    { GC_LOOP;
      { property * p = make_a_property_any(GC_OID((*(OBJECT(Call,x)->args))[1]));
        ...
        else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[106] the object ~S does not understand ~S")),
       ...
       { thing * _Zo = new_thing_class(_Zc,self->ident);
        if (INHERIT(_Zo->isa,Kernel._property))
         { if (CLREAD(property,_Zo,restrictions)->length > 0)
           close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[188] the property ~S is already defined")),
...
{ if ((INHERIT(owner_any(get_symbol(self->ident)),Kernel._class)) && 
  ...
  { { OID Result = 0;
      { OID  V_CL0072;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[107] class re-definition is not valid: ~S")),
      ...
      return (Result);} 
        ...
        (self->arg->open == -1)) 
    { { OID Result = 0;
        { OID  V_CL0073;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[109] the parent class ~S of ~S is closed")),
              ...
              { OID  v = CNULL;
                ...
                { ClaireType * rt = GC_OBJECT(ClaireType,extract_type_any(GC_OID((*Kernel.range)(x))));
                  ...
                  if ((v != CNULL) && 
                   ...
                   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[108] default(~S) = ~S does not belong to ~S")),
                  ...
                  { ClaireBoolean * g0074I;
                    ...
                    if (g0074I == CTRUE) close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[181] cannot overide a slot for a closed property ~S")),
...
{ GC_BIND;
  if (inherit_ask_class(self->arg->isa,Language._Call) != CTRUE)
   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[110] wrong signature definition ~S")),
  ...
  { OID Result = 0;
    { property * p = make_a_property_any(_oid_(self->arg->selector));
      ...
      if ((l->length == 1) && 
       ...
       { OID v_bag;
        ...
        { { Variable * _CL_obj = ((Variable *) GC_OBJECT(Variable,new_object_class(Language._Variable)));
            (_CL_obj->pname = symbol_I_string2(CSTRING("XfakeParameter")));
       ...
       { OID  rtest;
        ...
        if (rtest != CNULL)
         { restriction * r = OBJECT(restriction,rtest);
          tformat_string(CSTRING("--- WARNING ! [186] conflict between ~S and ~S is dangerous since ~S is closed\n"),1,list::alloc(3,_oid_(m),
            _oid_(r),
            _oid_(p)));
          } 
        else ;} 
      ...
      if ((p == Kernel.close) && 
       ...
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[184] the close method ~S has a wrong range")),
      ...
      { bag *v_list; OID v_val;
        ...
        { v = (*(v_list))[CLcount];
          if (inherit_ask_class(OBJECT(ClaireObject,v)->isa,Language._Variable) != CTRUE)
           { OID  V_CL0076;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[111] wrong typed argument ~S")),
            ...
            v_val=_void_(V_CL0076);} 
              ...
              ++n;
              if (p == CNULL)
               close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[111] wrong typed argument ~S (~S)")),
    ...
    { ClaireObject *V_CC ;
      { (Language.LDEF->value= _oid_(list::empty(Kernel._any)));
        { OID  r = GC_OID(extract_pattern_any(x,Kernel.nil));
          if (r == CNULL)
           close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[112] wrong type expression ~S")),
      ...
      { ClaireBoolean * g0084I;
        ...
        if (g0084I == CTRUE) { Call * _CL_obj = ((Call *) GC_OBJECT(Call,new_object_class(Language._Call)));
         ...
         { if ((l->length == 2) && 
          ...
          else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[113] Wrong lambda definition lambda[~S]")),
                  ...
                  { OID  y = (*(l))[n];
                    ...
                    if (INHERIT(OWNER(y),Language._Call))
                     { if (((OBJECT(Call,y)->selector == Kernel._equal) ? ((OBJECT(Call,y)->args->length == 2) ? CTRUE: CFALSE): CFALSE) != CTRUE)
                       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[114] Wrong parametrization ~S")),
        ...
        { list * lv2 = GC_OBJECT(list,list::empty(Kernel._any));
          ...
          { lambda * lb = GC_OBJECT(lambda,lambda_I_list(lv2,GC_OID((*(OBJECT(bag,(*Core.args)(x))))[2])));
            ...
            { ClaireHandler c_handle = ClaireHandler();
              ...
              { c_handle.catchIt();{ princ_string(CSTRING("The type expression "));
                  print_any(x);
                  princ_string(CSTRING(" is not valid ... \n"));
                  princ_string(CSTRING("context: lambda = "));
                  print_any(_oid_(lb));
                  princ_string(CSTRING(", lvars = "));
                  { OID  g0093UU;
            ...
            if (inherit_ask_class(OWNER(ur),Kernel._type) != CTRUE)
             close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[115] the (resulting) range ~S is not a type")),
...
Cint  bit_vector_listargs2(listargs *l)
{ { Cint Result = 0;
    { Cint  d = 0;
      { ITERATE(x);
        for (START(l); NEXT(x);)
                ...
                { GC_LOOP;
                  { OID  g0097UU;
                    { if (INHERIT(OWNER(u),Kernel._integer))
                      ...
                      else { OID  V_CL0098;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[116] ~S not allowed in function!")),
        ...
        if (INHERIT(s->isa,Core._Interval))
         ...
         { OID gc_local;
          ...
          if ((INHERIT(s->isa,Core._Interval)) && 
           ...
           { (ar->params = _oid_(list::alloc(2,Core.size->fcall(((Cint) s2)),(((CLREAD(Interval,s,arg1)*(Core.size->fcall(((Cint) s2))))+CLREAD(Interval,s2,arg1))-1))));
            (ar->graph = make_copy_list_integer(((Core.size->fcall(((Cint) s)))*(Core.size->fcall(((Cint) s2)))),d));
            } 
          else { (ar->params = _oid_(Kernel._any));
              (ar->graph = make_list_integer(29,CNULL));
              } 
     ...
     Result = (*Language.eval_rule)(_oid_(self));
        ...
        { tuple * g0104 = make_filter_any(_Zcondition);
          ...
          if (INHERIT(OWNER(OBJECT(ClaireRelation,R)->if_write),Kernel._function))
           close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("cannot define a new rule on ~S which is closed")),
  ...
  { tuple *Result ;
    { ClaireObject *V_CC ;
      { OID  c;
        ...
        if ((INHERIT(OWNER(c),Language._Call)) && (((OBJECT(Call,c)->selector == Core.write) || 
         ...
         { ClaireRelation * R = OBJECT(ClaireRelation,(*(OBJECT(bag,(*Core.args)(c))))[1]);
          ...
          if (multi_ask_any(_oid_(R)) == CTRUE)
           close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[188] wrong event filter ~S for multi-valued relation")),
        ...
        else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[188] wrong event filter: ~S")),
              ...
              { { Call * _CL_obj = ((Call *) GC_OBJECT(Call,new_object_class(Language._Call)));
                  ...
                  { Call * g0112 = _CL_obj; 
                    ...
                    { OID v_bag;
                      ...
                      ((list *) g0113)->addFast(_string_(CSTRING("--- trigger ~A(~S,~S)\n")));
                      { { List * _CL_obj = ((List *) GC_OBJECT(List,new_object_class(Language._List)));
...
{ GC_BIND;
  { method * m = add_method_property(p,
    ...
    store_object(m,
      ...
      p->if_write,
      CFALSE);
    (m->functional = make_function_string(append_string(GC_STRING(string_I_symbol(p->name)),CSTRING("_write"))));
    } 
  GC_UNBIND;} 
...
{ if (boolean_I_any(_oid_(p->restrictions)) != CTRUE)
   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[185] cannot define an empty property ~S as an interface")),
    _oid_(list::alloc(1,_oid_(p))))));
  if ((uniform_property(p) != CTRUE) || 
      (OBJECT(restriction,(*(p->restrictions))[1])->domain->memq(_oid_(Kernel._float)) == CTRUE))
   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[185] cannot define an non-uniform property ~S as an interface")),
    _oid_(list::alloc(1,_oid_(p))))));
