...
{ GC_BIND;
  { Cint  n = (Core.pretty->cprevious+1);
...
{ if (Core.pretty->cprevious == 0)
   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[123] unbalanced use of print-in-string")),
    _oid_(Kernel.nil))));
  { char *Result ;
    { Cint  n = Core.pretty->cprevious;
      ...
      if ((boolean_I_any(_oid_(_Zprop)) == CTRUE) && 
       ...
       { list * l = _Zprop->srange;
        fcall1(_Zprop->evaluate,OBJECT(ClaireClass,(*(l))[1]),self,OBJECT(ClaireClass,(*(l))[2]));
          ...
          { c_handle.catchIt();{ princ_string(CSTRING("<unprintable:"));
...
{ GC_BIND;
  if (self == CNULL)
   princ_string(CSTRING("unknown"));
  else { ClaireClass * c = OWNER(self);
      Cint  n = c->params->length;
      if (n > 0)
       { print_any(_oid_(c));
        princ_string(CSTRING("("));
        { Cint  i = 1;
...
{ if (self == CTRUE)
   princ_string(CSTRING("true"));
  else princ_string(CSTRING("false"));
    } 
...
{ if ((Core.pretty->pbreak == CTRUE) && 
   ...
   { Cint  b_index = buffer_length_void();
    ClaireBoolean * missed = CFALSE;
    Cint  _Zl = Core.pretty->index;
...
ClaireBoolean * short_enough_integer(Cint self)
...
{ { OID Result = 0;
    if (belong_to(self,_oid_(y)) == CTRUE)
     Result = self;
    else { OID  V_CL0062;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[124] the value ~S does not belong to the range ~S")),
    ...
    { ClaireObject *V_CC ;
      { ClaireBoolean * g0063I;
        ...
        if (g0063I == CTRUE) V_CC = cast_I_bag(self,y);
          else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[124] the value ~S does not belong to subtype[~S²]")),
...
{ { OID Result = 0;
    { ITERATE(c);
      ...
      if ((OBJECT(ClaireClass,c)->instances->length != 0) || 
       ...
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[187] cannot declare ~S as ephemeral because of ~S")),
...
{ { OID Result = 0;
    { Cint  n = c->open;
      if (n == ClEnv->ephemeral)
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[125] ephemeral classes cannot be abstract")),
        _oid_(Kernel.nil))));
...
{ { OID Result = 0;
    { Cint  n = c->open;
      if (n == 3)
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[126] ephemeral classes cannot be set as final")),
        _oid_(Kernel.nil))));
      else if (n == 2)
       (c->open = 1);
      else if (n == 0)
...
{ { OID Result = 0;
    { Cint  n = p->open;
      if (n < 2)
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[127] ~S can no longer become abstract")),
...
{ close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("cannot execute C code: ~A")),
...
{ { OID Result = 0;
    { OID  V_CL0073;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("cannot execute ~A")),
        _oid_(list::alloc(1,_string_(s))))));
...
ClaireChar * nth_get_string(char *s,Cint n,Cint max)
{ { ClaireChar *Result ;
    { ClaireObject *V_CC ;
      if (n <= max)
       V_CC = _char_(s[n - 1]);
      else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("Buffer string access")),
          _oid_(Kernel.nil))));
...
void  nth_put_string(char *s,Cint n,ClaireChar *c,Cint max)
{ if (n <= max)
   (s[n - 1] = (char) c->ascii);
...
Cint  _plus_integer(Cint self,Cint x)
...
/* The c++ function for: -(self:integer,x:integer) [0] */
Cint  _dash_integer1(Cint self,Cint x)
{ return ((self-x));} 
...
Cint  _inf_inf_integer(Cint x,Cint y)
{ return ((x << y));} 


/* The c++ function for: >>(x:integer,y:integer) [0] */
Cint  _sup_sup_integer(Cint x,Cint y)
{ return ((x >> y));} 


/* The c++ function for: and(x:integer,y:integer) [0] */
Cint  and_integer(Cint x,Cint y)
{ return ((x & y));} 


/* The c++ function for: or(x:integer,y:integer) [0] */
Cint  or_integer(Cint x,Cint y)
{ return ((x | y));} 


// open-coded
/* The c++ function for: <(self:integer,x:integer) [0] */
ClaireBoolean * _inf_integer(Cint self,Cint x)
{ { ClaireBoolean *Result ;
    Result = ((self < x) ?
      CTRUE :
      CFALSE );
...
/* The c++ function for: <=(self:integer,x:integer) [0] */
ClaireBoolean * _inf_equal_integer(Cint self,Cint x)
{ { ClaireBoolean *Result ;
    Result = ((self <= x) ?
      CTRUE :
      CFALSE );
...
/* The c++ function for: >(self:integer,x:integer) [0] */
ClaireBoolean * _sup_integer(Cint self,Cint x)
{ { ClaireBoolean *Result ;
    Result = ((self > x) ?
      CTRUE :
      CFALSE );
...
ClaireBoolean * nth_integer(Cint self,Cint y)
{ { ClaireBoolean *Result ;
...
void  printFDigit_float_(OID g0092,Cint g0093)
...
void  printFDigit_float(double x,Cint i)
{ if (x < 0.0)
   { princ_string(CSTRING("-"));
...
void  printFDigit_integer(Cint x,Cint i)
{ if (i > 0)
   { Cint  f = _exp_integer(10,(i-1));
    Cint  d = (x/f);
    princ_integer(d);
    if (i > 1)
...
Cint  length_bag(bag *self)
{ return (self->length);} 


/* The c++ function for: nth(self:bag,x:integer) [RETURN_ARG] */
OID  nth_bag(bag *self,Cint x)
{ { OID Result = 0;
    if ((x > 0) && 
        (x <= self->length))
     Result = (*(self))[x];
    else { OID  V_CL0094;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[41] nth[~S] out of scope for ~S")),
...
OID  nth_get_bag(bag *self,Cint x)
{ return ((*(self))[x]);} 
...
{ { OID Result = 0;
    ...
    else { OID  V_CL0095;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[183] min of empty set is undefined")),
          _oid_(Kernel.nil))));
...
{ { OID Result = 0;
    ...
    else { OID  V_CL0096;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[183] max of empty set is undefined")),
          _oid_(Kernel.nil))));
...
{ { OID Result = 0;
    ...
    else { OID  V_CL0097;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[41] car(nil) is undefined")),
          _oid_(Kernel.nil))));
...
OID  nth_set_list(list *self,Cint x,OID y)
{ { OID Result = 0;
    if ((x <= 0) || 
        (x > self->length))
     { OID  V_CL0098;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[41] nth[~S] out of scope for ~S")),
        _oid_(list::alloc(2,x,_oid_(self))))));
...
{ { OID Result = 0;
    if (self->length > 0)
     Result = (*(self))[1];
    else { OID  V_CL0100;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[41] car(nil) is undefined")),
          _oid_(Kernel.nil))));
...
void  quicksort_list(list *self,method *f,Cint n,Cint m)
{ if (m > n)
   { OID  x = (*(self))[n];
    if (m == (n+1))
...
list * make_copy_list_integer(Cint n,OID d)
{ GC_BIND;
  { list *Result ;
...
void  nth_equal_array(OID *self,Cint x,OID y)
{ if (belong_to(y,_oid_(of_array(self))) != CTRUE)
   close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("type mismatch for array update ~S, ~S")),
    _oid_(list::alloc(2,y,_array_(self))))));
  else if ((x > 0) && 
      (x <= self[0]))
   nth_put_array(self,x,y);
  else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("nth[~S] out of scope for ~S")),
