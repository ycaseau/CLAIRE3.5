  ...
  { OID Result = 0;
    { OID  _Zread = _oid_(Kernel.emptySet);
      ...
      if (INHERIT(OWNER(self),Kernel._bag))
       { Cint  i = 1;
        Cint  g0104 = OBJECT(bag,self)->length;
        { OID gc_local;
          while ((i <= g0104))
          { princ_integer(i);
              ...
              if (INHERIT(OWNER(val),Kernel._bag))
               { if (OBJECT(bag,val)->length < 10)
                 pretty_print_any(val);
                    ...
                    pretty_print_any(_string_(CSTRING("...")));
                    } 
                  } 
...
{ GC_BIND;
  { OID  self = (*(old))[1];
    if ((INHERIT(OWNER(_Zread),Language._Call)) && (OBJECT(Call,_Zread)->selector == Kernel.put))
     { Cint  n = (*(OBJECT(bag,(*Core.args)(_Zread))))[1];
      symbol * s = extract_symbol_any((*(OBJECT(bag,(*Core.args)(_Zread))))[2]);
      if (inherit_ask_class(OWNER(n),Kernel._integer) != CTRUE)
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[128] ~S should be an integer")),
    ...
    else princ_string(CSTRING("=> given to inspector is wrong.\n"));
      } 
  { OID Result = 0;
...
OID  get_from_integer_any(OID self,Cint n)
{ { OID Result = 0;
    if (INHERIT(OWNER(self),Kernel._bag))
     { if ((n > 0) && 
       ...
       Result = (*Kernel.nth)(self,
        n);
          ...
          princ_string(CSTRING(" in not a good index for "));
          print_any(self);
          princ_string(CSTRING(".\n"));
          Result = self;
          } 
        ...
        if ((n > 0) && 
         ...
         { OID v_rec;
          ...
          Result=Kernel.get->super(Kernel._slot,2);} 
            ...
            princ_string(CSTRING(" is not a good index for "));
            print_any(self);
            princ_string(CSTRING(".\n"));
            Result = self;
            } 
...
{ GC_BIND;
  if (INHERIT(OWNER(self),Kernel._property))
   ...
   (ClEnv->ctrace = fopen_string(string_v(self),CSTRING("w")));
  else if (INHERIT(OWNER(self),Kernel._integer))
   (ClEnv->verbose = self);
  else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[129] trace not implemented on ~S\n")),
...
{ GC_BIND;
  ...
  else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[130] untrace not implemented on ~S\n")),
      _oid_(list::alloc(1,self)))));
    { OID Result = 0;
    Result = self;
    GC_UNBIND; return (Result);} 
...
{ GC_BIND;
  if (5 <= (Kernel.if_write->trace_I+ClEnv->verbose))
   { OID  p = GC_OID(get_property(Kernel.ctrace,ClEnv));
    ...
    princ_string(CSTRING(","));
    print_any(v);
    princ_string(CSTRING(") by an update "));
    print_any(_oid_(R));
    princ_string(CSTRING("("));
    print_any(x);
    princ_string(CSTRING(") "));
...
{ if (ClEnv->debug_I != -1)
   { (ClEnv->debug_I = -1);
    princ_string(CSTRING("debugger removed\n"));
    } 
  else { (ClEnv->debug_I = 0);
      (ClEnv->ctrace = EXPORT((ClairePort *),Reader.STDOUT->value));
      princ_string(CSTRING("debugger installed\n"));
      } 
    } 
...
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  { Cint  top = ClEnv->debug_I;
    Cint  s = ClEnv->step_I;
    Cint  t = ClEnv->trace_I;
    (ClEnv->step_I = 0);
    (ClEnv->trace_I = 0);
    ...
    if (((OBJECT(ClaireBoolean,(*Kernel._sup)(GC_OID(ClEnv->stack[top]),
     ...
     { OID  j = GC_OID(ClEnv->stack[top]);
      OID  num_args = GC_OID((*Kernel._dash)(GC_OID(ClEnv->stack[GC_OID((*Core._plus)(j,
          2))]),
        1));
      Cint  start = ClEnv->stack[GC_OID((*Core._plus)(j,
        3))];
      OID  m = GC_OID(ClEnv->stack[GC_OID((*Core._plus)(j,
        1))]);
      princ_string(CSTRING("break in "));
      print_any(m);
      princ_string(CSTRING("("));
...
void  up_integer(Cint x)
{ GC_RESERVE(1);  // HOHO v3.0.55 optim !
  if (x > 0)
   { list * indices = Kernel.nil;
    Cint  ind = Reader._starmaxd_star->value;
    { OID gc_local;
...
void  where_integer(Cint x)
{ { Cint  j = Reader._starcurd_star->value;
    Cint  stack_level = 0;
    { while (((j > 0) && 
          ((x > 0) && 
            (ClEnv->debug_I > 0))))
...
void  print_debug_info_integer(Cint index,Cint stack_level,Cint cur_index)
{ GC_BIND;
  { Cint  num_args = ((ClEnv->stack[(index+2)])-1);
    Cint  start = ClEnv->stack[(index+3)];
    OID  m = GC_OID(ClEnv->stack[(index+1)]);
    princ_string(CSTRING("debug["));
    princ_integer((cur_index+stack_level));
    princ_string(CSTRING("]>"));
    { Cint  x = 1;
      Cint  g0111 = stack_level;
      { OID gc_local;
        while ((x <= g0111))
        { princ_string(CSTRING(">"));
          ++x;
...
OID  Show_integer(Cint n)
{ GC_BIND;
  { OID Result = 0;
    { Cint  i = Reader._starcurd_star->value;
      { OID gc_local;
        Result= _oid_(CFALSE);
        while (((i > 0) && 
            (n > 0)))
        { GC_LOOP;
          { Cint  num_args = ((ClEnv->stack[(i+2)])-1);
            Cint  start = ClEnv->stack[(i+3)];
            princ_string(CSTRING("["));
            princ_integer(start);
            princ_string(CSTRING(" - "));
            princ_integer(i);
            princ_string(CSTRING("]: p = "));
            print_any(GC_OID(ClEnv->stack[(i+1)]));
            princ_string(CSTRING(", narg = "));
            print_any(num_args);
            princ_string(CSTRING(" \n"));
            { Cint  j = 0;
              Cint  g0113 = num_args;
        ...
        { OID  nargs = GC_OID(ClEnv->stack[(j+2)]);
          ...
          if (Kernel._method == m->isa)
           { princ_string(CSTRING("debug["));
            ...
            else { princ_string(CSTRING("<compiled:"));
                print_any(_oid_(CLREAD(restriction,m,module_I)));
                princ_string(CSTRING(">"));
                } 
              princ_string(CSTRING(")\n"));
            } 
          else { princ_string(CSTRING("debug["));
              princ_integer(((Reader._starindex_star->value)+stack_level));
              princ_string(CSTRING("] > "));
              print_any(_oid_(z));
              princ_string(CSTRING(" -> "));
...
{ princ_string(CSTRING(") : [(i)n,(o)ut,e(x)it,(t)race,(b)reakpoint]\n"));
  { Cint  m = 1;
    ...
    if (((char) c->ascii) == 'i')
     ...
     close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("exit stepper")),
      _oid_(Kernel.nil))));
...
{ { ClairePort * p = fopen_string(s,CSTRING("w"));
    Cint  n = size_class(Reader._measure);
    use_as_output_port(p);
    princ_string(CSTRING("// log file produced on "));
...
void  addLog_integer_(Cint g0127,OID g0128,OID g0129,OID g0130,Cint g0131)
...
void  addLog_integer(Cint i,double x,double y,double n,Cint s)
{ if (size_class(Reader._measure) == s)
  ...
  else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("logMeasure not compatible with current set (~A vs ~A)")),
    ...
    { ClaireObject *V_CC ;
      { ClaireObject * x = GC_OBJECT(ClaireObject,p->reified);
        if (OWNER(_oid_(x)) == Reader._PRcount)
         ...
         close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[131] Cannot profile a reified property ~S")),
  ...
  { ClaireObject * x = GC_OBJECT(ClaireObject,p->reified);
    if (INHERIT(x->isa,Reader._PRcount))
     { print_any(_oid_(p));
      ...
      princ_string(CSTRING(" calls -> "));
      princ_integer(CLREAD(PRcount,x,rtime));
      princ_string(CSTRING(" clock tics\n"));
      } 
    } 
  GC_UNBIND;} 
...
Cint  PRtime_property(property *p)
{ { Cint Result = 0;
...
Cint  PRcounter_property(property *p)
{ { Cint Result = 0;
    { ClaireObject * x = p->reified;
      Result = ((INHERIT(x->isa,Reader._PRcount)) ?
          ...
          { OID V_C;{ OID gc_local;
              ...
              { GC_LOOP;
                { ClaireBoolean * g0135I;
                  { OID  g0136UU;
                    { Cint  i = 1;
                      Cint  g0134 = min_integer(10,l->length);
                      { OID gc_local;
