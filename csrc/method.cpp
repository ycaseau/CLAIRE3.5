...
OID  eval_message_property(property *self,ClaireObject *r,Cint start,ClaireBoolean *int_ask)
{ { OID Result = 0;
...
OID  noeval_message_property2(property *self,Cint start)
{ GC_BIND;
  { OID Result = 0;
...
OID  execute_method(method *self,Cint start,ClaireBoolean *int_ask)
{ GC_BIND;
  { OID Result = 0;
    { OID  val = CNULL;
      Cint  n = self->domain->length;
...
{ GC_BIND;
  { ClaireHandler c_handle = ClaireHandler();
    ...
    { c_handle.catchIt();tformat_string(CSTRING("---- WARNING: inline definition of ~S is wrong\n"),0,list::alloc(1,_oid_(self)));
      } 
  ...
  { ClaireObject * s = GC_OBJECT(ClaireObject,_at_property1(self,OWNER(_oid_(x))));
    if (Kernel._slot == s->isa)
     { if (belong_to(y,_oid_(CLREAD(restriction,s,range))) != CTRUE)
       ...
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[132] Cannot change ~S(~S)")),
...
void  put_property1(property *p,ClaireObject *x,Cint n,ClaireClass *s,OID y)
{ tformat_string(CSTRING("are you still using this dead thing (put instead of update) ??? !! \n"),0,list::empty());
  update_property(p,
    x,
    n,
    s,
...
void  update_property(property *p,ClaireObject *x,Cint n,ClaireClass *s,OID y)
{ GC_BIND;
    ...
    if ((r != NULL) && 
     ...
     { if (INHERIT(r->isa,Kernel._property))
       { ClaireObject * s = GC_OBJECT(ClaireObject,_at_property1(((property *) r),OWNER(y)));
        ...
        else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[133] Inversion of ~S(~S,~S) impossible")),
       ...
       { OID  old_y = GC_OID(get_table(((table *) r),y));
        Cint  i = get_index_table1(((table *) r),y);
        if (r->multivalued_ask != CFALSE)
...
{ if (INHERIT(r->isa,Kernel._property))
   ...
   { OID  l = get_table(((table *) r),x);
    Cint  i = get_index_table1(((table *) r),x);
    OID  v;
    if (INHERIT(OWNER(l),Kernel._bag))
     v = _oid_(delete_bag(((r->store_ask == CTRUE) ?
      copy_bag(OBJECT(bag,l)) :
...
void  add_I_property(property *self,ClaireObject *x,Cint n,OID y)
...
ClaireBoolean * add_value_property(property *self,ClaireObject *x,Cint n,bag *l,OID y)
{ GC_BIND;
  { ClaireBoolean *Result ;
    ...
    { property * p = x->selector;
      ...
      if (p->dictionary == CTRUE)
       { if (uniform_restriction(x) == CTRUE)
         { if (ClEnv->verbose == 4)
           tformat_string(CSTRING("--- hashinsert ~S \n"),0,list::alloc(1,_oid_(x)));
      ...
      if ((ix > 0) && 
       ...
       { ClaireClass * c = domain_I_restriction(x);
        ...
        else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("Cannot create a non-uniform restriction ~S of interface ~S")),
          ...
          { GC_LOOP;
            { list * l2 = OBJECT(restriction,(*(l))[i])->domain;
              if (tmatch_ask_list(x->domain,l2) == CTRUE)
               ...
               tformat_string(CSTRING("~S and ~S are conflicting"),2,list::alloc(2,(*(l))[1],_oid_(x)));
              } 
            ++i;
            GC_UNLOOP;} 
          } 
...
ClaireBoolean * matching_ask_list(list *l,Cint n,Cint m)
{ { ClaireBoolean *Result ;
    { Cint  x = (m-n);
      Cint  z = l->length;
      if ((z == x) && 
...
ClaireBoolean * vmatch_ask_any(OID t,OID x,Cint n)
{ { ClaireBoolean *Result ;
     ...
     { ClaireBoolean *v_and;
      { v_and = vmatch_ask_any(_oid_(OBJECT(Param,t)->arg),x,n);
        if (v_and == CFALSE) Result =CFALSE; 
            ...
            { Cint  i = 1;
              Cint  g0031 = OBJECT(Param,t)->params->length;
       ...
       { ClaireBoolean *v_and;
        { v_and = ((OBJECT(bag,t)->length == OBJECT(bag,x)->length) ? CTRUE : CFALSE);
          if (v_and == CFALSE) Result =CFALSE; 
              ...
              { Cint  i = 1;
                Cint  g0032 = OBJECT(bag,x)->length;
     ...
     { ClaireBoolean *v_and;
      { v_and = tmatch_ask_any(t,_oid_(OBJECT(Param,t2)->arg),l);
        if (v_and == CFALSE) Result =CFALSE; 
            ...
            { Cint  i = 1;
              Cint  g0039 = OBJECT(Param,t2)->params->length;
       ...
       { ClaireBoolean *v_and;
        { v_and = ((OBJECT(bag,t2)->length == OBJECT(bag,t)->length) ? CTRUE : CFALSE);
          if (v_and == CFALSE) Result =CFALSE; 
              ...
              { Cint  i = 1;
                Cint  g0040 = OBJECT(bag,t2)->length;
...
ClaireObject * find_which_property(property *p,Cint n,ClaireClass *c)
{ { ClaireObject *Result ;
    if (p->dictionary == CTRUE)
...
ClaireObject * find_which_list(list *l,ClaireClass *c,Cint n,Cint m)
{ { ClaireObject *Result ;
    { OID V_C;{ ITERATE(r);
        V_C= Kernel.cfalse;
        for (START(l); NEXT(r);)
...
ClaireObject * find_which_class(ClaireClass *c,list *l,Cint n,Cint m)
{ { ClaireObject *Result ;
    { OID V_C;{ ITERATE(r);
        V_C= Kernel.cfalse;
        for (START(l); NEXT(r);)
