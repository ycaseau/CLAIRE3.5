...
{ GC_BIND;
  { OID Result = 0;
    Result = _string_(append_string(CSTRING("3."),GC_STRING(string_I_float(ClEnv->version))));
...
OID  about_void()
{ princ_string(CSTRING("CLAIRE v3."));
  princ_float(ClEnv->version);
  princ_string(CSTRING(" Copyright (C) 1994-2013 Yves Caseau. All Rights Reserved.\n"));
  princ_string(CSTRING("use and redistribution in source code or binary forms are permitted\n"));
  princ_string(CSTRING("resale is not permitted without the explicit agreement of Yves Caseau\n"));
  princ_string(CSTRING("THIS SOFTWARE IS PROVIDED AS IS AND WITHOUT ANY WARRANTY, INCLUDING,\n"));
  princ_string(CSTRING("WITHOUT LIMITATION, THE IMPLIED WARRANTIES OF MERCHANTABILTY AND FITNESS\n"));
  princ_string(CSTRING("FOR A PARTICULAR PURPOSE\n"));
  return (Kernel.ctrue);} 
...
void  push_debug_property(property *prop,Cint arity,Cint start)
{ GC_BIND;
  { Cint  i = ClEnv->index;
    Cint  n = ClEnv->trace_I;
    if ((n > 0) && 
    ...
    if (get_table(Core.StopProperty,_oid_(prop)) != CNULL)
     { { ClaireBoolean * g0046I;
        ...
        if (g0046I == CTRUE) close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("stop as required in ~S(~A)")),
...
void  pop_debug_property(property *self,Cint n,OID val)
{ GC_BIND;
  { Cint  v = ClEnv->debug_I;
    if (v > 0)
     { if (n != 0)
...
void  tr_indent_boolean(ClaireBoolean *return_ask,Cint n)
{ if (return_ask == CTRUE)
   { princ_string(CSTRING("["));
    princ_integer(n);
    princ_string(CSTRING("]"));
...
{ GC_BIND;
  { ClaireObject * s = GC_OBJECT(ClaireObject,_at_property1(self,OWNER(_oid_(x))));
    if (boolean_I_any(_oid_(s)) != CTRUE)
     ...
     close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[134] Cannot apply add to ~S")),
      _oid_(list::alloc(1,_oid_(self))))));
    else { Cint  n = CLREAD(slot,s,index);
...
{ { OID Result = 0;
    { OID  p = a->params;
      if (belong_to(x,_oid_(a->domain)) != CTRUE)
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[135] ~S does not belong to the domain of ~S")),
      ...
      { OID  v;
        ...
        else { Cint  i = index_table(a,x);
            v = (*(a->graph))[i];
            } 
          if ((v != CNULL) || 
            (belong_to(v,_oid_(a->range)) == CTRUE))
         Result = v;
        else { OID  V_CL0051;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[138] the value ~S(~S) is unknown !")),
...
{ { OID Result = 0;
    { OID  p = a->params;
      if (belong_to(x,_oid_(a->domain)) != CTRUE)
       Result = CNULL;
      else { Cint  i = get_index_table1(a,x);
          Result = (*(a->graph))[i];
          } 
        } 
    return (Result);} 
...
ClaireBoolean * add_value_array(table *self,Cint n,bag *l,OID y)
{ GC_BIND;
...
{ { OID  p = self->params;
    Cint  i = get_index_table1(self,x);
      ...
      if (INHERIT(OWNER(p),Kernel._list))
       { if (((belong_to(x,(*(((bag *) a->domain)))[1]) == CTRUE) ? ((belong_to(y,(*(((bag *) a->domain)))[2]) == CTRUE) ? CTRUE: CFALSE): CFALSE) != CTRUE)
        ...
        if ((v != CNULL) || 
          (belong_to(v,_oid_(a->range)) == CTRUE))
       Result = v;
      else { OID  V_CL0054;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("~S(~S) is unknown !")),
            _oid_(list::alloc(2,_oid_(a),x)))));
...
Cint  get_index_table1(table *a,OID x)
{ { Cint Result = 0;
    { OID  p = a->params;
...
Cint  get_index_table2(table *a,Cint x,Cint y)
{ { Cint Result = 0;
  ...
  { ClaireRelation * r1 = OBJECT(ClaireRelation,_Zr1);
    ...
    if ((_inf_equal_type(r1->domain,GC_OBJECT(ClaireType,((multi_ask_any(_oid_(r2)) == CTRUE) ?
        ...
        (_inf_equal_type(r2->domain,GC_OBJECT(ClaireType,((multi_ask_any(_oid_(r1)) == CTRUE) ?
          ...
          r1->range ))) != CTRUE))
     close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[137] ~S and ~S cannot be inverses for one another")),
...
{ GC_BIND;
  princ_string(CSTRING("**** An error has occurred.\n"));
...
{ GC_BIND;
  princ_string(CSTRING("****[138] The value of "));
  print_any(GC_OID(self->wrong));
  princ_string(CSTRING("("));
  print_any(GC_OID(self->arg));
  princ_string(CSTRING(") is unknown"));
  GC_UNBIND;} 
...
{ GC_BIND;
  princ_string(CSTRING("****[139] "));
  print_any(GC_OID(self->cause));
  princ_string(CSTRING(": range error, "));
  print_any(GC_OID(self->arg));
  princ_string(CSTRING(" does not belong? to "));
  print_any(GC_OID(self->wrong));
  princ_string(CSTRING(".\n"));
  GC_UNBIND;} 
...
{ GC_BIND;
  { OID  p = GC_OID(self->selector);
    if (boolean_I_any(_oid_(OBJECT(property,p)->restrictions)) != CTRUE)
     { princ_string(CSTRING("[140] The property "));
      print_any(p);
      princ_string(CSTRING(" is not defined (was applied to "));
      print_any(GC_OID(self->arg));
      princ_string(CSTRING(").\n"));
      } 
    else { princ_string(CSTRING("****[141] "));
        print_any(GC_OID(self->arg));
        princ_string(CSTRING(" is a wrong arg list for "));
        print_any(p);
        princ_string(CSTRING(".\n"));
        } 
      } 
...
{ princ_string(CSTRING("****[142] return called outside of a loop (for or while)."));
  } 
...
{ GC_BIND;
  { Cint  n = self->index;
    princ_string(CSTRING("**** An internal error ["));
    princ_integer(n);
    princ_string(CSTRING("] has occured:\n"));
    { char * g0057UU;
      if (n == 1)
       g0057UU = CSTRING("dynamic allocation, item is too big (~S)");
      else if (n == 2)
       g0057UU = CSTRING("dynamic allocation, too large for available memory (~S)");
      else if (n == 3)
       g0057UU = CSTRING("object allocation, too large for available memory (~S)");
      else if (n == 5)
       g0057UU = CSTRING("nth[~S] outside of scope for ~S");
      else if (n == 7)
       g0057UU = CSTRING("Skip applied on ~S with a negative argument ~S");
      else if (n == 8)
       g0057UU = CSTRING("List operation: cdr(()) is undefined");
      else if (n == 9)
       g0057UU = CSTRING("String buffer is full: ~S");
      else if (n == 10)
       g0057UU = CSTRING("Cannot create an imported entity from NULL reference");
      else if (n == 11)
       g0057UU = CSTRING("nth_string[~S]: string too short~S");
      else if (n == 12)
       g0057UU = CSTRING("Symbol Table table full");
      else if (n == 13)
       g0057UU = CSTRING("Cannot create a subclass for ~S [~A]");
      else if (n == 16)
       g0057UU = CSTRING("Temporary output string buffer too small");
      else if (n == 17)
       g0057UU = CSTRING("Bag Type Error: ~S cannot be added to ~S");
      else if (n == 18)
       g0057UU = CSTRING("definition of ~S is in conflict with an object from ~S");
      else if (n == 19)
       g0057UU = CSTRING("Integer overflow");
      else if (n == 20)
       g0057UU = CSTRING("Integer arithmetic: division/modulo of ~A by 0");
      else if (n == 21)
       g0057UU = CSTRING("Integer to character: ~S is a wrong value");
      else if (n == 22)
       g0057UU = CSTRING("Cannote create a string with negative length ~S");
      else if (n == 23)
       g0057UU = CSTRING("Not enough memory to instal claire");
      else if (n == 24)
       g0057UU = CSTRING("execution stack is full [~A]");
      else if (n == 26)
       g0057UU = CSTRING("Wrong usage of time counter [~A]");
      else if (n == 27)
       g0057UU = CSTRING("internal garbage protection stack overflow");
      else if (n == 28)
       g0057UU = CSTRING("the multivalued status of ~S is not compatible with ~S");
      else if (n == 29)
       g0057UU = CSTRING("There is no module ~S");
      else if (n == 30)
       g0057UU = CSTRING("Attempt to read a private symbol ~S");
      else if (n == 31)
       g0057UU = CSTRING("External function not compiled yet");
      else if (n == 32)
       g0057UU = CSTRING("Too many arguments (~S) for function ~S");
      else if (n == 33)
       g0057UU = CSTRING("Exception handling: stack overflow");
      else if (n == 34)
       g0057UU = CSTRING("User interrupt: EXECUTION ABORTED");
      else if (n == 35)
       g0057UU = CSTRING("reading char '~S': wrong char: ~S");
      else if (n == 36)
       g0057UU = CSTRING("cannot open file ~A");
      else if (n == 37)
       g0057UU = CSTRING("world stack is full");
      else if (n == 38)
       g0057UU = CSTRING("Undefined access to ~S");
      else if (n == 39)
       g0057UU = CSTRING("cannot convert ~S to an integer");
      else if (n == 40)
       g0057UU = CSTRING("integer multiplication overflow with ~S and ~S");
      else if (n == 41)
       g0057UU = CSTRING("wrong NTH access on ~S and ~S");
      else if (n == 42)
       g0057UU = CSTRING("Wrong array[~S] init value: ~S");
      else if (n == 43)
       g0057UU = CSTRING("Defeasible addition on list ~S requires pre-allocation (size ~S)");
      else if (n == 50)
       g0057UU = CSTRING("C++ imported error (~S) : ~S");
      else { (self->value = n);
          g0057UU = CSTRING("What the hell is this ! [code: ~S^]");
          } 
...
{ princ_string(CSTRING("A contradiction has occured."));
  } 
      ...
      { GC_LOOP;
        { ClaireChar * m = _char_(s[(n+1) - 1]);
          ...
          if ('A' == ((char) m->ascii))
           ...
           close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[143] ~I not allowed in format")),
...
OID  tformat_string(char *self,Cint i,list *l)
{ if (i <= ClEnv->verbose) 
  { { OID Result = 0;
      { ClairePort * p = (use_as_output_port(ClEnv->ctrace));
