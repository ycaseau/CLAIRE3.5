...
{ { OID Result = 0;
    { OID  V_CL0002;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[144] evaluate(~S) is not defined")),
...
{ GC_BIND;
  { OID Result = 0;
    ...
    else { OID  V_CL0003;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[145] the symbol ~A is unbound")),
    ...
    { OID  i = self->index;
      if (i != CNULL)
       Result = ClEnv->stack[GC_OID((*Core._plus)(GC_OID(ClEnv->base),
        i))];
      else { OID  V_CL0005;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[146] The variable ~S is not defined")),
...
OID  lexical_build_any(OID self,list *lvar,Cint n)
  ...
  { { OID Result = 0;
    ...
    if (INHERIT(OWNER(self),Language._Variable))
     { if (OBJECT(Variable,self)->index == (CNULL))
       close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[145] the symbol ~A is unbound")),
     ...
     { Cint  _Zn = OBJECT(bag,self)->length;
      { OID gc_local;
        while ((_Zn > 0))
    ...
    { ClaireObject *V_CC ;
      if (INHERIT(OWNER(self),Kernel._unbound_symbol))
       ...
       { V_CC = (((OBJECT(ClaireBoolean,self)) == CTRUE) ?
          symbol_I_string2(CSTRING("true")) :
          symbol_I_string2(CSTRING("nil")) );
        } 
      else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[147] a name cannot be made from ~S")),
...
{ GC_BIND;
  { property *Result ;
    { ClaireObject *V_CC ;
      ...
      else close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[148] Wrong selector: ~S, cannot make a property\n")),
          _oid_(list::alloc(1,self)))));
...
OID  indent_integer(Cint limit)
{ { OID Result = 0;
    { Cint  x = buffer_length_void();
      { Result= _oid_(CFALSE);
        while ((x < limit))
        { princ_string(CSTRING(" "));
          ++x;
...
OID  printbox_bag1(bag *self,Cint start,Cint finish,char *s)
{ { OID Result = 0;
    { Cint  i = 1;
...
{ return (printbox_bag1(self,buffer_length_void(),Core.pretty->width,CSTRING(", ")));} 
...
{ if (((INHERIT(OWNER(self),Language._Call)) && (((INHERIT(OBJECT(Call,self)->selector->isa,Kernel._operation)) ? ((comp != CTRUE) ? ((OBJECT(Call,self)->args->length == 2) ? CTRUE: CFALSE): CFALSE): CFALSE) != CTRUE)) || 
   ...
   print_any(self);
  else { Cint  _Zl = Core.pretty->index;
      princ_string(CSTRING("("));
      set_level_integer(1);
      print_any(self);
      princ_string(CSTRING(")"));
