/***** CLAIRE Compilation of file /Users/ycaseau/claire/v3.5/src/meta/read.cl 
    ...
    { OID Result = 0;
      if (x == _oid_(Reader._ask))
       ...
       { OID  y = GC_OID(nexte_meta_reader(cnext_meta_reader(r)));
        if (y == _oid_(Kernel._equal))
         ...
         { OID  w = GC_OID(nexte_meta_reader(r));
          if (w == _oid_(Reader._equal_sup))
           ...
           Serror_string(CSTRING("[149] wrong keyword (~S) after ~S"),list::alloc(2,w,y));
        ...
        else Serror_string(CSTRING("[150] Illegal use of :~S after ~S"),list::alloc(2,y,x));
          } 
          ...
          else Serror_string(CSTRING("[152] Separation missing between ~S \nand ~S [~S?]"),list::alloc(3,x,
              y,
              _oid_(e)));
            } 
        GC_UNBIND; return (Result);} 
  ...
  { OID Result = 0;
    { Cint  n = skipc_meta_reader(r);
      if (n == 41)
       ...
       Serror_string(CSTRING("[153] eof inside an expression"),Kernel.nil);
      else if (n == 59)
       { { OID gc_local;
          while (((firstc_meta_reader(r) != r->eof) && 
              (firstc_meta_reader(r) != 10)))
                ...
                { GC_LOOP;
                  if (firstc_meta_reader(r) == 60)
                   { OID  y = GC_OID(nexte_meta_reader(cnext_meta_reader(r)));
                    ...
                    else Serror_string(CSTRING("[154] ~S<~S not allowed"),list::alloc(2,x,y));
                      } 
  ...
  { OID Result = 0;
    if (firstc_meta_reader(r) == 40)
        ...
        if (_Za2 == _oid_(Reader.in))
         { _Za3= GC_OID(nexte_meta_reader(r));
          if (equal(GC_OID(nexte_meta_reader(r)),GC_OID(Reader.OR->value)) != CTRUE)
           Serror_string(CSTRING("[155] missing | in exists / forall"),Kernel.nil);
          } 
        else if (equal(_Za2,r->comma) == CTRUE)
         cnext_meta_reader(r);
        else Serror_string(CSTRING("[156] wrong use of exists(~S ~S ..."),list::alloc(2,_oid_(v),_Za2));
     ...
     { OID  s = GC_OID(readset_meta_reader(r,GC_OID(nexte_meta_reader(cnext_meta_reader(r)))));
      ...
      else Serror_string(CSTRING("[157] ~S cannot follow list{"),list::alloc(1,s));
        } 
     ...
     { OID  _Za1 = GC_OID(read_ident_port(cnext_meta_reader(r)->fromp));
      if (inherit_ask_class(OWNER(_Za1),Kernel._class) != CTRUE)
       Serror_string(CSTRING("[158] wrong type in call ~S@~S"),list::alloc(2,val,_Za1));
      if (firstc_meta_reader(r) == 40)
       Result = readcall_meta_reader(r,val,_Za1);
      else Serror_string(CSTRING("[159] missing ( after ~S@~S"),list::alloc(2,val,_Za1));
        } 
    else Result = val;
      GC_UNBIND; return (Result);} 
  } 
...
{ { OID Result = 0;
    if (firstc_meta_reader(cnext_meta_reader(r)) == 47)
     { Cint  val = firstc_meta_reader(cnext_meta_reader(r));
      next_meta_reader(r);
      Result = val;
      } 
    ...
    else Serror_string(CSTRING("[160] wrong use of special char #"),Kernel.nil);
      return (Result);} 
  } 
...
{ GC_BIND;
  { OID Result = 0;
    { OID  x = GC_OID(nexts_meta_reader(r,e));
      if (boolean_I_any(stop_ask_integer(firstc_meta_reader(r))) != CTRUE)
       Result = x;
      else Serror_string(CSTRING("[161] Missing keyword ~S after ~S"),list::alloc(2,_oid_(e),x));
        } 
    GC_UNBIND; return (Result);} 
  } 
...
OID  nexts_I_meta_reader2(meta_reader *r,Cint e)
{ GC_BIND;
  { OID Result = 0;
    { OID  x = GC_OID(nexts_meta_reader(r,Reader.none));
      ...
      else Serror_string(CSTRING("[162] Missing separator ~S after ~S"),list::alloc(2,_oid_(char_I_integer(e)),x));
        } 
    GC_UNBIND; return (Result);} 
  } 
...
OID  nexts_I_meta_reader3(meta_reader *r,keyword *e,Cint n)
{ GC_BIND;
  { OID Result = 0;
    { OID  x = GC_OID(nexts_meta_reader(r,e));
      if ((firstc_meta_reader(r) == n) || 
          (boolean_I_any(stop_ask_integer(firstc_meta_reader(r))) != CTRUE))
       Result = x;
      else Serror_string(CSTRING("[163] wrong separator ~S after ~S"),list::alloc(2,_oid_(char_I_integer(firstc_meta_reader(r))),x));
        } 
    GC_UNBIND; return (Result);} 
  } 
...
{ GC_BIND;
  { OID Result = 0;
    { Cint  i = get_string(s,_char_(']'));
      Cint  k = included_string(s,CSTRING("//"),CTRUE);
      Cint  m = strlen(s);
      Cint  cx = firstc_meta_reader(r);
      ...
      else { princ_string(CSTRING("trace("));
          { Cint  j = 2;
            Cint  g0018 = (i-1);
            { while ((j <= g0018))
              { princ_char(_char_(s[j - 1]));
