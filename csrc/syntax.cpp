/***** CLAIRE Compilation of file /Users/ycaseau/claire/v3.5/src/meta/syntax.cl 
    ...
    if (y == _oid_(Reader.as))
     ...
     { if (INHERIT(OWNER(x),Core._global_variable))
       ...
       { if ((INHERIT(OWNER(z),Language._Call)) && (((OBJECT(Call,z)->selector == Kernel.add) || 
        ...
        else Serror_string(CSTRING("[164] ~S cannot be assigned with :="),list::alloc(1,x));
          } 
...
OID  operand_I_any(OID x,Cint n)
{ GC_BIND;
  { OID Result = 0;
    if (INHERIT(OWNER(x),Language._Or))
     { if (n == 1)
...
{ GC_BIND;
  { OID Result = 0;
    { OID  _Zdef = GC_OID(nexts_I_meta_reader3(r,Reader.in,44));
      ...
      else Serror_string(CSTRING("[165] ~S is illegal after a let"),list::alloc(1,_Zdef));
        } 
    GC_UNBIND; return (Result);} 
  } 
...
OID  readlet_star_meta_reader(meta_reader *r,list *l,Cint n,keyword *e)
{ if (n > l->length) 
  { { OID Result = 0;
...
{ GC_BIND;
  { OID Result = 0;
    { OID  _Zdef = GC_OID(nexts_I_meta_reader3(r,Reader.in,44));
      ...
      else Serror_string(CSTRING("[165] ~S is illegal after a when"),list::alloc(1,_Zdef));
        } 
    GC_UNBIND; return (Result);} 
  } 
...
OID  readif_meta_reader(meta_reader *r,Cint e)
{ GC_BIND;
  { OID Result = 0;
  ...
  { OID Result = 0;
    { OID  _Zv = GC_OID(nexte_meta_reader(r));
      if (skipc_I_meta_reader(r) != 40)
       Serror_string(CSTRING("[166] Missing ( after case ~S"),list::alloc(1,_Zv));
      { Case * _Zx;
        ...
        { OID gc_local;
          ...
          { GC_LOOP;
            ...
            if ((boolean_I_any(stop_ask_integer(firstc_meta_reader(r))) != CTRUE) && 
             ...
             Serror_string(CSTRING("[167] missing ) or , after ~S"),list::alloc(1,_oid_(_Zx)));
            GC_UNLOOP;} 
          } 
        next_meta_reader(r);
        if ((e != Reader.none) && 
         ...
         Serror_string(CSTRING("[161] missing ~S after ~S"),list::alloc(2,_oid_(e),_oid_(_Zx)));
        Result = _oid_(_Zx);
        } 
      } 
    GC_UNBIND; return (Result);} 
  ...
  { { OID Result = 0;
    ...
    { OID Result = 0;
      { OID  _Za2 = GC_OID(nexte_meta_reader(r));
          ...
          { Iteration * g0047 = _CL_obj; 
            ...
            { list * _Zbind = GC_OBJECT(list,bind_I_meta_reader(r,v));
              ...
              { if (equal(GC_OID(nexte_meta_reader(r)),GC_OID(Reader.OR->value)) != CTRUE)
                 Serror_string(CSTRING("[168] missing | in selection"),Kernel.nil);
        ...
        else Serror_string(CSTRING("[169] missing separation between ~S and ~S"),list::alloc(2,_Za1,_Za2));
          } 
      GC_UNBIND; return (Result);} 
    } 
  } 
...
{ { OID Result = 0;
    if (INHERIT(OWNER(x),Kernel._unbound_symbol))
     { OID  V_CL0050;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[170] cannot use ~S in a set constant")),
        _oid_(list::alloc(1,x)))));
      
      Result=_void_(V_CL0050);} 
    else if (INHERIT(OWNER(x),Language._Variable))
     { OID  V_CL0051;close_exception(((general_error *) (*Core._general_error)(_string_(CSTRING("[170] cannot use a variable (~S) in a set constant")),
        _oid_(list::alloc(1,x)))));
...
OID  nextseq_meta_reader(meta_reader *r,Cint e)
{ GC_BIND;
  { OID Result = 0;
    if (firstc_meta_reader(r) == e)
     { next_meta_reader(r);
      Result = _oid_(list::empty());
        ...
        else Serror_string(CSTRING("[171] Read the character ~S inside a sequence"),list::alloc(1,_oid_(char_I_integer(firstc_meta_reader(r)))));
          } 
      GC_UNBIND; return (Result);} 
  } 
...
OID  readblock_meta_reader(meta_reader *r,OID x,Cint e)
{ GC_BIND;
  skipc_meta_reader(r);
  { OID Result = 0;
    if (equal(x,r->paren) == CTRUE)
     ...
     Serror_string(CSTRING("[172] the sequence ...~S must end with ~A"),list::alloc(2,x,_oid_(char_I_integer(e))));
     ...
     Serror_string(CSTRING("[173] Expression starting with else"),Kernel.nil);
    else if (keyword_ask_any(x) == CTRUE)
      ...
      if (x == _oid_(Reader.PRINTF))
       ...
       { { ClaireBoolean * g0059I;
          ...
          if (g0059I == CTRUE) { list * l2 = OBJECT(ClaireClass,x)->params;
              Cint  n = ((INHERIT(l2->isa,Kernel._list)) ?
                l2->length :
                0 );
              if ((*Kernel.length)(l) == n)
               { list * V_CL0065;{ list * i_bag = list::empty(Kernel.emptySet);
                ...
                l=_oid_(V_CL0065);} 
              else Serror_string(CSTRING("[174] Wrong instantiation list ~S(~S..."),list::alloc(2,x,GC_OID(_oid_(list::alloc(1,l)))));
                } 
            } 
    ...
    if (equal(y,Reader.triangle->value) == CTRUE)
     ...
     { ClaireBoolean * table_ask = ((INHERIT(OWNER(x),Language._Call)) ?
      ...
      if ((table_ask == CTRUE) ? (w == _oid_(Reader.L__equal)) : ((equal(w,Reader.arrow->value) == CTRUE) || 
       ...
       ;else Serror_string(CSTRING("[149] wrong keyword (~S) after ~S"),list::alloc(2,w,z));
  ...
  { Defclass *Result ;
    { OID  c = GC_OID(verify_any(_oid_(Kernel._class),GC_OID(read_ident_port(r->fromp)),_oid_(Language._Defclass)));
        ...
        y = _CL_obj;
          ...
          { OID gc_local;
            ...
            { ClaireBoolean * g0079I;
              ...
              if (g0079I == CTRUE) Serror_string(CSTRING("[175] Wrong form ~S in ~S(~S)"),list::alloc(3,y1,
                  c,
                  l));
                } 
            } 
      ...
      if ((INHERIT(OWNER(x),Language._Call)) && (OBJECT(Call,x)->selector == Kernel.nth))
       ...
       Serror_string(CSTRING("[176] Missing ] after ~S "),list::alloc(1,_oid_(y)));
